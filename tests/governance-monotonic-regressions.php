...
$passed = 0; $failed = 0;
function f13_governance_check( $condition, $message ) { global $passed, $failed; if ( $condition ) { ++$passed; echo "PASS: $message\n"; } else { ++$failed; echo "FAIL: $message\n"; } }

swi_test_reset();
$stored = SWI_Config::defaults(); $stored['frequency_days'] = 45; $stored['eligible_routes'] = array( '/', '/about' ); $stored['starts_at'] = '2026-08-10T00:00:00+00:00'; $stored['ends_at'] = '2026-08-20T00:00:00+00:00'; $GLOBALS['swi_test_options'][ SWI_Config::OPTION_CONFIG ] = $stored;
add_filter( 'swi_runtime_config', static function ( $config ) { $config['frequency_days'] = 30; $config['eligible_routes'] = array( '*' ); $config['suppressed_prefixes'] = array( '/custom-suppression' ); $config['starts_at'] = '2026-08-01T00:00:00+00:00'; $config['ends_at'] = '2026-08-30T00:00:00+00:00'; $config['analytics_enabled'] = 1; $config['brand_claim'] = 'Companion-owned copy is forbidden'; return $config; } );
...
swi_test_reset();
$stored = SWI_Config::defaults(); $stored['frequency_days'] = 30; $stored['eligible_routes'] = array( '/', '/about' ); $stored['starts_at'] = '2026-08-10T00:00:00+00:00'; $stored['ends_at'] = '2026-08-20T00:00:00+00:00'; $GLOBALS['swi_test_options'][ SWI_Config::OPTION_CONFIG ] = $stored;
add_filter( 'swi_runtime_config', static function ( $config ) { $config['frequency_days'] = 60; $config['eligible_routes'] = array( '/about' ); $config['suppressed_prefixes'][] = '/private-task'; $config['starts_at'] = '2026-08-15T00:00:00+00:00'; $config['ends_at'] = '2026-08-18T00:00:00+00:00'; return $config; } );
$config = SWI_Config::get();
...
swi_test_reset();
$stored = SWI_Config::defaults(); $GLOBALS['swi_test_options'][ SWI_Config::OPTION_CONFIG ] = $stored; $GLOBALS['swi_test_options'][ SWI_Config::OPTION_WRITE_LOCK ] = array( 'token' => 'other-writer', 'expires_at' => time() + 30 ); $raw = $stored; $raw['frequency_days'] = 60; $result = SWI_Config::save( $raw, 1 );
...
swi_test_reset();
$stored = SWI_Config::defaults(); $GLOBALS['swi_test_options'][ SWI_Config::OPTION_CONFIG ] = $stored; $GLOBALS['swi_test_options'][ SWI_Config::OPTION_WRITE_LOCK ] = array( 'token' => 'stale-writer', 'expires_at' => time() - 1 ); $raw = $stored; $raw['frequency_days'] = 60; $result = SWI_Config::save( $raw, 1 );
...
swi_test_reset();
$stored = SWI_Config::defaults(); $GLOBALS['swi_test_options'][ SWI_Config::OPTION_CONFIG ] = $stored; add_filter( 'swi_runtime_config', static function ( $config ) { $config['frequency_days'] = 90; return $config; } ); $raw = $stored; $raw['frequency_days'] = 45; $result = SWI_Config::save( $raw, 1 );
