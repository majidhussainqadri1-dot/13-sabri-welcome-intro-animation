...
function apply_filters( $tag, $value ) {
	...
	if ( empty( $GLOBALS['swi_test_filters'][ $tag ] ) ) return $value;
	ksort( $GLOBALS['swi_test_filters'][ $tag ] );
	foreach ( $GLOBALS['swi_test_filters'][ $tag ] as $callbacks ) foreach ( $callbacks as $callback ) { $args[0] = call_user_func_array( $callback, $args ); }
	return $args[0];
}
...
function swi_test_reset() {
	...
	$GLOBALS['swi_test_actions'] = array();
	$GLOBALS['swi_test_current_user'] = 0;
	$GLOBALS['swi_test_admin'] = $GLOBALS['swi_test_ajax'] = $GLOBALS['swi_test_json'] = false;
	$GLOBALS['swi_test_feed'] = $GLOBALS['swi_test_robots'] = $GLOBALS['swi_test_trackback'] = $GLOBALS['swi_test_embed'] = false;
	$_SERVER['REQUEST_URI'] = '/';
}
