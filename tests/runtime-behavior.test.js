...
function storage(initial = {}, throws = false) {
  ...
  return { getItem(k) { if (throws) throw new Error('blocked'); return values.has(k) ? values.get(k) : null; }, setItem(k, v) { if (throws) throw new Error('blocked'); values.set(k, String(v)); }, removeItem(k) { if (throws) throw new Error('blocked'); values.delete(k); }, values };
}
...
function createRuntime(options = {}) {
  ...
  intro.setAttribute('hidden', ''); intro.setAttribute('data-preview', options.preview ? '1' : '0'); intro.setAttribute('data-preview-state', options.previewState || 'default'); intro.setAttribute('data-preview-variant', options.previewVariant || 'full'); intro.setAttribute('data-replay', options.replay ? '1' : '0'); intro.setAttribute('data-frequency-days', String(options.frequency || 30)); intro.setAttribute('data-duration', String(options.duration === undefined ? 0 : options.duration)); intro.setAttribute('data-reduced-duration', '900'); intro.setAttribute('data-config-version', '3'); intro.setAttribute('data-experience-version', '1.1.0'); intro.setAttribute('data-cookie-name', 'swi_seen_at_v1'); intro.setAttribute('data-session-key', 'swi_seen_session_v1'); intro.setAttribute('data-local-key', 'swi_seen_at_v1'); intro.setAttribute('data-claim-key', 'swi_claim_v1'); intro.setAttribute('data-never-key', 'swi_never_show_v1'); intro.setAttribute('data-profile-key', 'swi_a11y_profile_v1'); intro.setAttribute('data-rest-url', options.restUrl || ''); intro.setAttribute('data-rest-nonce', options.restNonce || ''); intro.setAttribute('data-event-nonce', options.eventNonce || ''); intro.setAttribute('data-analytics', options.analytics ? '1' : '0'); intro.setAttribute('data-account-authoritative', options.accountAuthoritative ? '1' : '0'); intro.setAttribute('data-user-id', String(options.userId || 0)); intro.setAttribute('data-account-seen-at', '0'); intro.setAttribute('data-account-never-show', '0'); intro.setAttribute('data-account-experience', ''); intro.setAttribute('data-accessibility-profile', options.profile || 'auto'); intro.setAttribute('data-adaptive', '1'); intro.setAttribute('data-never-show-enabled', '1'); intro.setAttribute('data-replay-enabled', '1'); intro.setAttribute('data-version-replay', '1'); intro.setAttribute('data-guest-reconcile', '1'); intro.setAttribute('data-instant-exit', '1'); intro.setAttribute('data-data-saver-static', '1'); intro.setAttribute('data-performance-circuit', '1'); intro.setAttribute('data-performance-budget', '120');
  intro.querySelector = (q) => q === '[data-swi-close]' ? close : q === '[data-swi-continue]' ? cont : q === '[data-swi-skip]' ? skip : q === '[data-swi-never]' ? never : q === '.swi-ambient' ? ambient : null;
...
test('Continue persists version-aware timestamp, restores background and focus', () => { const r = createRuntime({ accountAuthoritative: false, userId: 0 }); r.cont.listeners.get('click')(); const closeTimer = [...r.timers.values()].find((x) => x.delay === 220); assert.ok(closeTimer); closeTimer.fn(); assert.equal(r.intro.isConnected, false); assert.match(r.cookie(), /^swi_seen_at_v1=/); const seen = JSON.parse(r.localStorage.values.get('swi_seen_at_v1')); assert.ok(Number(seen.ts) > 0); assert.equal(seen.version, '1.1.0'); assert.equal(r.previous.hasAttribute('inert'), false); assert.equal(r.document.activeElement, r.previous); assert.equal(r.events.at(-1).type, 'swi:completed'); });
