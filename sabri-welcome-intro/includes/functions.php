...
if ( ! function_exists( 'swi_approval_capability' ) ) { function swi_approval_capability() { $capability = apply_filters( 'swi_founder_approval_capability', 'manage_options' ); return is_string( $capability ) && '' !== sanitize_key( $capability ) ? sanitize_key( $capability ) : 'manage_options'; } }
...
if ( ! function_exists( 'swi_current_user_can_approve' ) ) {
	/**
	 * Founder visual acceptance is not ordinary WordPress administration.
	 * File 00 (or an explicitly approved institutional adapter) must assert the
	 * current user's canonical Founder identity. File 13 never infers Founder
	 * status from manage_options, display name, role slug, email or user ID.
	 */
	function swi_current_user_can_approve() {
		$user_id = get_current_user_id();
		if ( $user_id <= 0 ) { return false; }
		$founder_asserted = (bool) apply_filters( 'swi_founder_identity_asserted', false, $user_id );
		if ( ! $founder_asserted ) { return false; }
		return current_user_can( swi_approval_capability() );
	}
}
...
if ( ! function_exists( 'swi_get_welcome_intro_contract' ) ) {
	...
	function swi_get_welcome_intro_contract() {
		return array(
			...
			'capability_filter' => 'swi_manage_capability', 'founder_identity_filter' => 'swi_founder_identity_asserted', 'approval_capability_filter' => 'swi_founder_approval_capability', 'runtime_config_filter' => 'swi_runtime_config',
